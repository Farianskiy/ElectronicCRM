...
public interface ICatalogImportBatchRepository
{
    ...
    Task<IReadOnlyCollection<CatalogImportRow>> GetRowsAsync(Guid batchId, CatalogImportRowStatus? status, int skip, int take, CancellationToken cancellationToken = default);

    Task<int> CountRowsAsync(Guid batchId, CatalogImportRowStatus? status, CancellationToken cancellationToken = default);
