...
import { getSuggestionEvidence } from "@/features/catalogRecognition/api/learningProvenance";
...
export function SuggestionCard({
  ...
  const evidence = useQuery({
    queryKey: ["suggestion-evidence", suggestion.id],
    queryFn: () => getSuggestionEvidence(suggestion.id),
    enabled: isRecognitionLearning,
  ...
  const productTypeValue = suggestion.productTypeName
    ...
    : (suggestion.productTypeCode ?? "Все типы товаров");
  ...
  return (
    <article className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        ...
        <section className="rounded-2xl border border-white/10 bg-black/20 p-4">
          <h4 className="font-semibold text-white">Где применяется</h4>

          <div className="mt-3 grid gap-2 text-sm">
            <InfoRow label="Тип товара" value={productTypeValue} />
            <InfoRow
              label="Ограничение по типу"
              value={
      ...
      {isRecognitionLearning && (
        <section className="my-4 rounded border border-white/10 p-4">
          {evidence.data && !evidence.isError && (
            <p>
              Основания: {evidence.data.occurrenceCount} наблюдений,{" "}
              {evidence.data.distinctProductCount} разных имён.{" "}
              {evidence.data.sufficientEvidence
                ? canReview
                  ? "Учебных оснований достаточно; требуется оценка влияния."
                  : "Учебных оснований достаточно."
                : "Недостаточно для одобрения."}{" "}
              {evidence.data.needsReview &&
                "Основания изменились — требуется пересмотр."}
            </p>
          )}
          {evidence.error && <p>Не удалось проверить основания.</p>}
          <button type="button" onClick={() => void evidence.refetch()}>
            Обновить основания
          </button>
        </section>
      )}
      {evidence.data?.evaluationReportId && (
        <EvaluationReportPanel
          reportId={evidence.data.evaluationReportId}
          kind="dictionary"
        />
      )}
      {canReview && (
        <>
          <DictionarySuggestionApprovalForm
            suggestion={suggestion}
            evidenceRevision={evidence.data?.revision}
